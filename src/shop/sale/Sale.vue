...
const getImages = (sale) => {
  // 1. imageUrl 단건이면 배열로 감싸서 반환
  if (typeof sale.imageUrl === 'string') {
    return [sale.imageUrl]
  }

  // 2. imageUrls 문자열 (쉼표 구분)
  if (typeof sale.imageUrls === 'string') {
    return sale.imageUrls.split(',')
  }

  // 3. imageUrls 배열
  if (Array.isArray(sale.imageUrls)) {
    return sale.imageUrls
  }
...
const getCondition = (sale) => {
  return sale.conditionName || sale.productList?.[0]?.conditionName
}
...
const getMinPrice = (sale) => {
  if (sale.price && sale.period) {
    return { price: sale.price, period: sale.period }  // SliceRes 전용
  }

  if (Array.isArray(sale.priceList) && sale.priceList.length) {
...
/>
          ...
          <div class="card-body text-center">
            <h6 class="card-title fw-bold">
              {{ sale.name }}
              <span v-if="getCondition(sale)"
      class="badge ms-2"
      :class="conditionColorClass(getCondition(sale))">
  {{ getCondition(sale) }}
</span>
    ...
    <div class="text-center mt-4">
      <button v-if="!endReached && !loading" class="btn btn-primary" @click="loadMore">
        더 보기
      </button>
